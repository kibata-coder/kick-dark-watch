...
function normalizeMatches(raw: any): Match[] {
  ...
  return items.map((m) => {
    ...
    return {
      id: m.id, title: m.title, status: deriveStatus(dateMs), time: formatKickoff(dateMs), date: dateMs,
      home: { name: m.teams?.home?.name || t.home || "Home", logo: m.teams?.home?.badge },
      away: { name: m.teams?.away?.name || t.away || "Away", logo: m.teams?.away?.badge },
    ...
    <div className="flex items-center justify-between gap-3">
      <div className="flex min-w-0 items-center gap-3">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-muted ring-1 ring-border p-1">
          {src ? <img src={src} alt="logo" className="h-full w-full object-contain" /> : <Trophy className="h-4 w-4 text-muted-foreground" />}
        </div>
        <span className="truncate text-sm font-medium text-foreground">{name}</span>
      </div>
      {score !== undefined && <span className="font-mono text-lg font-bold">{score}</span>}
    </div>
  );
}
...
function FootballPage() {
  ...
  const load = async () => {
    ...
    } catch (e: any) { setError(e?.message || "Could not load matches"); }
  ...
  const handleWatch = async (m: Match) => {
    ...
    try {
      ...
      if (!url) throw new Error("No stream available");
      setStreamUrl(url);
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-end justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight sm:text-3xl">Live Football</h2>
      ...
      {error && (
        <div className="mb-6 flex items-start gap-3 rounded-lg border border-destructive/40 bg-destructive/10 p-4 text-sm"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0 text-destructive" /><div><p className="font-medium">Error</p><p className="text-muted-foreground">{error}</p></div></div>
      )}
          ...
          {matches.map((m) => (
            <Card key={String(m.id)} className="group flex flex-col overflow-hidden border-border/60 bg-card/80 backdrop-blur transition-all hover:border-primary/40">
              <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-3">
                <div className="flex items-center gap-2 text-xs text-muted-foreground"><Trophy className="h-3.5 w-3.5" /><span className="truncate">{m.league?.name || "Football"}</span></div>
                <StatusBadge status={m.status} />
              </CardHeader>
              <CardContent className="space-y-3">
                <TeamRow name={m.home?.name} logo={m.home?.logo} score={m.home?.score} />
                <TeamRow name={m.away?.name} logo={m.away?.logo} score={m.away?.score} />
              </CardContent>
              <CardFooter className="mt-auto pt-3">
                <Button onClick={() => handleWatch(m)} className="w-full font-semibold" size="sm"><Play className="mr-1.5 h-4 w-4 fill-current" /> Watch Stream</Button>
              </CardFooter>
            </Card>
          ))}
        </div>
        ...
        <DialogContent className="max-w-5xl border-border bg-card p-0 sm:rounded-2xl">
          <DialogHeader className="border-b border-border/60 px-5 py-4">
            <DialogTitle className="flex items-center gap-3 text-base">
              <span className="truncate">{selected?.home?.name} <span className="text-muted-foreground">vs</span> {selected?.away?.name}</span>
            </DialogTitle>
          </DialogHeader>
