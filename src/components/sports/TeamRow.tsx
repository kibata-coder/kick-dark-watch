...
function TeamRowImpl({ name, logo, score, isPortrait }: { name?: string; logo?: string; score?: string | number; isPortrait?: boolean }) {
  const flag = countryFlagUrl(name) ?? null;
  const initial: string | null = logo && logo.trim() ? logo : flag;
  const [src, setSrc] = useState<string | null>(initial);
  return (
    <div className="flex items-center gap-3">
      <div className={`flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-muted ring-1 ring-border ${isPortrait ? "" : "p-1"}`}>
        {src ? (
          <img
            src={src}
            alt=""
