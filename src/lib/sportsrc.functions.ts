...
const CACHE_TTL = 5 * 60 * 1000; // 5-minute memory caching system
...
export const getMatches = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    // We have completely removed the autoAssignDaddyChannelId hack.
    // The data is now passed cleanly to the frontend exactly as the API provides it.
...
export const getMatchDetail = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    // Return cached stream details if valid
    if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
